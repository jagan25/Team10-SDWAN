...
def createPE(yFile, yFileName, logFile):
    ...
    if "vms" in yFile:
        ...
        for vm in yFile["vms"]:
            ...
            # create provider vm
            command = "sudo ansible-playbook " + CREATE_VM_SCRIPT + " -e hypervisor=" + yFile['hypervisorType'] +" -e vm_name="+str(vm['PE_name']) +" -e mem="+str(vm['mem'])+" -e vcpu="+str(vm['vcpu']) +" -e network="+ controller_net +" -vvv"
            subprocess.call([command],shell=True)
