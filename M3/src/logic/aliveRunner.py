            ...
            if float(p['lastPing']) < time.time()-60:
                ...
                if "true" in str(output):
                   #see if new vms have to be spun up
                    # can be used to alert based on log messages
                    logFile.write(time.strftime("%Y%m%d-%H%M%S")+"   Heavy Load on container: "+p['name']+"\n")
                    ...
                    print(output)
                    logFile.write(time.strftime("%Y%m%d-%H%M%S")+"   Self Healing : "+p['name']+"\n")
