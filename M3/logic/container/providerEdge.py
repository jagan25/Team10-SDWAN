...
def createPE(yFile, logFile):
    ...
    if "containers" in yFile:
        ...
        for c in yFile["containers"]:
            ...
            #print(container)
            command = "sudo ansible-playbook " + CREATE_CONTAINER_SCRIPT + " -e hypervisor=" + yFile['hypervisorType'] +" -e container="+ container + " -e image=" + c['image']
            #print(command)
            subprocess.call([command],shell=True)
            ...
            command = "sudo ansible-playbook " + CREATE_DOC_CONN_SCRIPT + " -e hypervisor=" + yFile['hypervisorType'] + " -e veth1=" +veth1+ " -e veth2="+veth2+" -e bridge_name="+ controller_br + " -e container=" + str(container) + " -e option=run_dhclient -e option2=none"
